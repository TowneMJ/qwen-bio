"""
Improved Synthetic Genetics Training Data Generator
...
# Molecular genetics topics for initial sample
TOPICS = {
    "molecular_genetics": [
...
GENERATION_PROMPT = """You are an expert biology professor creating exam questions. Your questions will be reviewed by a PhD molecular biologist, so accuracy is critical.
...
2. SIMPLE AND DIRECT: Prefer straightforward questions over clever ones. The goal is to test knowledge, not trick the student.

3. AVOID ARITHMETIC: Do not write questions requiring multi-step calculations. Conceptual understanding only.

4. ONE CLEAR ANSWER: There must be exactly one defensible correct answer. All distractors must be clearly wrong to an expert. If you cannot construct such a question, pick a different aspect of the topic.

5. ANSWER OPTIONS: Provide exactly 10 options (A-J) to match MMLU-Pro format. Wrong answers should be plausible to a novice but clearly incorrect to an expert.

6. REASONING STRUCTURE - Solve the problem step by step as a student would:
   ...
   Do NOT start with the answer and work backward. Your reasoning must naturally lead to your conclusion.

7. SELF-CHECK: Before outputting, verify:
   - Does your reasoning genuinely support your chosen answer (not the other way around)?
...
{{
    ...
    "concept_tested": "One sentence describing what this question tests",
...
Only output questions where your confidence is HIGH. If you cannot create a high-confidence question on this topic, say so instead of guessing.
...
def save_dataset(questions: list, filename: str = "improved_genetics_qa.jsonl"):
...
def convert_to_chat_format(questions: list, filename: str = "improved_genetics_chat.jsonl"):
...
def main():
    ...
    print("Improved Genetics Training Data Generator")
    print("="*50)
    print(f"Model: {MODEL}")
    print(f"Output directory: {OUTPUT_DIR}")
    print(f"Topics: {list(TOPICS['molecular_genetics'])}")
    
    # Generate 2 questions per topic = 20 questions for review
    print("\n" + "="*50)
    print("Starting generation (2 questions per topic, 10 topics = 20 questions)...")
    print("="*50)
    ...
    if questions:
        # Save raw Q&A format
        save_dataset(questions, "improved_genetics_qa.jsonl")
        
        # Save chat format for training
        convert_to_chat_format(questions, "improved_genetics_chat.jsonl")
        
        print(f"\n{'='*50}")
        print(f"Generation complete!")
        print(f"Total questions generated: {len(questions)}")
        print(f"Success rate: {len(questions)}/20 ({100*len(questions)/20:.0f}%)")
        print(f"{'='*50}")
